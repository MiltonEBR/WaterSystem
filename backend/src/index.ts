...
const getData = async () => {
  const res = await axios.get(`//waterservices.usgs.gov/nwis/iv/?format=json&sites=${sites.join(',')}&parameterCd=${paramCodes.join(',')}&siteType=${siteArgs.type}&siteStatus=${siteArgs.status}`)

  const retreived = new Set<string>()
  for(let series of res.data.value.timeSeries){
    retreived.add(series.name as string)
  }

  console.log(Array.from(retreived))
}
